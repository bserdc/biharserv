...
const fetch = (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
  if (typeof input === 'string' && input.startsWith('/')) return globalThis.fetch(`${API_BASE_URL}${input}`, init);
...
export const api = {
  ...
  async patchNotice(id: string, data: Partial<NoticeCircular>): Promise<{ success: boolean; message: string; notice?: NoticeCircular }> {
    const res = await fetch(`/api/notices/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
  ...
  async publishNotice(id: string): Promise<{ success: boolean; message: string }> {
    const res = await fetch(`/api/notices/${id}/publish`, { method: 'POST' });
    return res.json();
  },

  async unpublishNotice(id: string): Promise<{ success: boolean; message: string }> {
    const res = await fetch(`/api/notices/${id}/unpublish`, { method: 'POST' });
    return res.json();
  },
