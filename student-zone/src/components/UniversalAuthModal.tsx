...
export const UniversalAuthModal: React.FC<UniversalAuthModalProps> = ({
  ...
  const [authRole, setAuthRole] = useState<'student' | 'admin'>(initialTab);
  ...
  // Demo autofill for Admin
  const handleFillAdminDemo = (demoUser: string, demoPass: string, demoRole: AdminRole) => {
    setUsername(demoUser);
    setPassword(demoPass);
    setAdminRole(demoRole);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {authRole === 'admin' && (
          <div>
            {/* Quick Demo Officer Pills */}
            <div className="mb-4 p-3 bg-slate-900 text-slate-100 rounded-xl border border-slate-800">
              <div className="flex items-center justify-between text-[11px] font-bold text-amber-300 mb-1.5">
                <span className="flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                  Council Officer Credentials (1-Click Fill)
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                <button
                  type="button"
                  onClick={() => handleFillAdminDemo('admin@bsedrc.gov.in', 'admin123', 'SUPER_ADMIN')}
                  className="px-2 py-1 bg-slate-800 hover:bg-amber-500 hover:text-slate-950 border border-slate-700 rounded-lg text-[11px] font-mono font-bold transition-colors"
                >
                  Super Admin (Dr. Rajeshwar)
                </button>
                <button
                  type="button"
                  onClick={() => handleFillAdminDemo('controller@bsedrc.gov.in', 'bsedrc@2026', 'EXAM_CONTROLLER')}
                  className="px-2 py-1 bg-slate-800 hover:bg-amber-500 hover:text-slate-950 border border-slate-700 rounded-lg text-[11px] font-mono font-bold transition-colors"
                >
                  Exam Controller (Prof. Sunita)
                </button>
                <button
                  type="button"
                  onClick={() => handleFillAdminDemo('officer@bsedrc.gov.in', 'pass123', 'DATA_OFFICER')}
                  className="px-2 py-1 bg-slate-800 hover:bg-amber-500 hover:text-slate-950 border border-slate-700 rounded-lg text-[11px] font-mono font-bold transition-colors"
                >
                  Data Officer (Manoj Jha)
                </button>
              </div>
            </div>
