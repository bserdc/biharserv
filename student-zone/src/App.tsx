...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<string>(isEmbeddedNoticeView || requestedTab === 'notices' ? 'notices' : 'student-register');
  ...
  const [authModalInitialTab, setAuthModalInitialTab] = useState<'student' | 'admin'>('student');

  // Initialize Sessions from LocalStorage
  useEffect(() => {
    try {
      // 1. Admin Session
      const savedAdminToken = localStorage.getItem('bsedrc_admin_token');
      const savedAdminUserStr = localStorage.getItem('bsedrc_admin_user');
      if (savedAdminToken && savedAdminUserStr) {
        const parsed = JSON.parse(savedAdminUserStr);
        setAdminUser(parsed);
        api.adminVerify(savedAdminToken).then((res) => {
  ...
  const handleAdminLogout = async () => {
    ...
    localStorage.removeItem('bsedrc_admin_token');
    localStorage.removeItem('bsedrc_admin_user');
    setAdminUser(null);
  ...
  const handleOpenAuthModal = (initialTab: 'student' | 'admin' = 'student') => {
        ...
        {isProtectedAdminTab && !adminUser && (
          <div className="max-w-2xl mx-auto my-12 bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 shadow-xl text-center animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mt-8 pt-6 border-t border-slate-200 text-xs text-slate-500">
              Default demo officer credentials: <span className="font-mono font-bold text-slate-800">admin@bsedrc.gov.in</span> / <span className="font-mono font-bold text-slate-800">admin123</span>
            </div>
          </div>
        )}
