...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  const [username, setUsername] = useState('admin@bsedrc.gov.in');
  ...
  const handleFillDemo = (demoUser: string, demoPass: string, demoRole: AdminRole) => {
    ...
    try {
      ...
      if (res.success && res.admin) {
        ...
        if (rememberMe && res.token) {
          localStorage.setItem('bsedrc_admin_token', res.token);
          localStorage.setItem('bsedrc_admin_user', JSON.stringify(res.admin));
        }
        setTimeout(() => {
          setIsLoading(false);
      ...
      // Fallback local authentication if server connection is offline
      if (
        (username === 'admin' || username === 'admin@bsedrc.gov.in') &&
        password === 'admin123'
      ) {
        const fallbackAdmin: AdminUser = {
          ...
          token: `bsedrc_local_token_${Date.now()}`,
          last_login: new Date().toISOString(),
        };
        if (rememberMe) {
          localStorage.setItem('bsedrc_admin_token', fallbackAdmin.token!);
          localStorage.setItem('bsedrc_admin_user', JSON.stringify(fallbackAdmin));
        }
        setSuccessMessage('Authentication safal! Swagat hai Administrator ji.');
        setTimeout(() => {
          setIsLoading(false);
        ...
        setIsLoading(false);
        setErrorMessage('Invalid username ya password. Niche diye demo credentials try karein.');
      }
    }
  };
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3.5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] font-bold uppercase text-slate-600 tracking-wider flex items-center gap-1.5">
                ...
                Demo Credentials (Ek-Click me Bharein):
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-left">
              <button
                ...
                onClick={() => handleFillDemo('admin@bsedrc.gov.in', 'admin123', 'SUPER_ADMIN')}
              ...
              >
                ...
                <div className="text-[10px] opacity-80 truncate">admin123</div>
              </button>

              <button
                type="button"
                id="fill-controller-btn"
                onClick={() => handleFillDemo('controller@bsedrc.gov.in', 'bsedrc@2026', 'EXAM_CONTROLLER')}
                className={`p-2 rounded-xl border text-[11px] font-medium transition-all ${
              ...
              >
                ...
                <div className="text-[10px] opacity-80 truncate">bsedrc@2026</div>
              </button>

              <button
                type="button"
                id="fill-officer-btn"
                onClick={() => handleFillDemo('officer@bsedrc.gov.in', 'pass123', 'DATA_OFFICER')}
                className={`p-2 rounded-xl border text-[11px] font-medium transition-all ${
                  role === 'DATA_OFFICER'
                    ? 'bg-amber-500 text-slate-950 border-amber-600 font-bold shadow-sm'
                    : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'
              ...
              >
                ...
                <div className="text-[10px] opacity-80 truncate">pass123</div>
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Jaise: admin@bsedrc.gov.in"
